...
func TestPrefetcherReturnsIntroReplyForNewClinicLink(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/data-proxy/query":
			if _, err := io.WriteString(w, `{"columns":["time","digest","plan_digest","query_time","parse_time","compile_time","cop_time","process_time","wait_time","total_keys","process_keys","result_rows","mem_max","disk_max","db","instance","index_names","prev_stmt","plan","decoded_plan","binary_plan","query"],"rows":[[1773973859.727374,"digest-1","plan-digest-1",7.5,0.1,0.2,2.5,1.5,0.3,1000,800,10,2048,0,"app","tidb-0","idx_a","begin","IndexLookUp_1 root 10.00","IndexLookUp(Build)","binary-plan-text","select * from t where a = 1"]]}`); err != nil {
...
func TestPrefetcherSupportsStatementDetailLink(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/data-proxy/query":
			if _, err := io.WriteString(w, `{"columns":["time","digest","plan_digest","query_time","parse_time","compile_time","cop_time","process_time","wait_time","total_keys","process_keys","result_rows","mem_max","disk_max","db","instance","index_names","prev_stmt","plan","decoded_plan","binary_plan","query"],"rows":[[1774222600,"digest-test-1","plan-digest-1",7.5,0.1,0.2,2.5,1.5,0.3,1000,800,10,2048,0,"test_db","tidb-0","idx_a","begin","IndexLookUp_1 root 10.00","IndexLookUp(Build)","binary-plan-text","select * from t where a = 1"]]}`); err != nil {
				t.Fatalf("write detail response: %v", err)
			}
		default:
			http.NotFound(w, r)
	...
	if got := enriched.RuntimeContext.Clinic.ClusterID; got != statementDetailTestClusterID {
		t.Fatalf("cluster ID = %q", got)
	}
	if got := enriched.RuntimeContext.Clinic.Digest; got != statementDetailTestDigest {
		t.Fatalf("digest = %q", got)
	}
	if got := enriched.RuntimeContext.Clinic.Database; got != statementDetailTestDB {
